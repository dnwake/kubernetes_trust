...
func TestCloneArgsGit(t *testing.T) {
	u, _ := url.Parse("git://github.com/dwake/docker-trust/external/github.com/docker/docker")
	args := cloneArgs(u, "/tmp")
	exp := []string{"clone", "--recursive", "--depth", "1", "git://github.com/dwake/docker-trust/external/github.com/docker/docker", "/tmp"}
	if !reflect.DeepEqual(args, exp) {
		t.Fatalf("Expected %v, got %v", exp, args)
	}
}
...
func TestCloneArgsStripFragment(t *testing.T) {
	u, _ := url.Parse("git://github.com/dwake/docker-trust/external/github.com/docker/docker#test")
	args := cloneArgs(u, "/tmp")
	exp := []string{"clone", "--recursive", "git://github.com/dwake/docker-trust/external/github.com/docker/docker", "/tmp"}
	if !reflect.DeepEqual(args, exp) {
		t.Fatalf("Expected %v, got %v", exp, args)
	}
}
